export const HANDLE = '<div class="aws-sso-item-handle"><svg xmlns="http://www.w3.org/2000/svg" width="1.25em" height="1.25em" viewBox="0 0 24 24"><path fill="currentColor" d="M20 9H4v2h16zM4 15h16v-2H4z"/></svg></div>';
...
export async function initializeAccountOrders(containers: NodeListOf<Element>) {
  const { [STORAGE_KEY]: accountIds = [] } = await chrome.storage.local.get(STORAGE_KEY) || { [STORAGE_KEY]: [] };
